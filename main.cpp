...
public:
    ...
    void save_data(string filename) {
        ...
        ofs << rebinned_data[0] << ' ' <<  no_pixels_rebinned[0];
        ofs.close();
    }
    ~OutputData() {
        free(rebinned_data);
...
//    const microstep_type dNf = par.dNf();
    ...
    out.save_data(par.output_filename);
...
int main() {
    ExperimentalParameters exp = load_xparm("/Users/arkadiy/ag/data/prussian blue/xds/XPARM.XDS");
    ReconstructionParameters par {\
    "/Users/arkadiy/ag/data/prussian blue/cbfs/14k_???.cbf",
    1, 360,
    {501,501,501},
    {-7,-7,-7},
